...
def to_table_html(nodes: list[dict]) -> str:
    ...
    # assigned (html, rowspan, node_id) at the top-left corner of that cell's span.
    grid: list[list[tuple[str, int, str | None] | None]] = [[None] * n_cols for _ in range(n_rows)]
    covered = [[False] * n_cols for _ in range(n_rows)]

    for i, ing in enumerate(ingredient_rows):
        grid[i][0] = (_ingredient_cell(ing), 1, ing["id"])
        covered[i][0] = True
    ...
    for c, layer in enumerate(op_layers, start=1):
        ...
        for top, bottom, op in placed:
            grid[top][c] = (_operation_cell(op), bottom - top + 1, op["id"])
    ...
    for r in range(n_rows):
        ...
        for c in range(n_cols):
            ...
            if cell is None:
                continue
            content, span, node_id = cell
            rowspan_attr = f' rowspan="{span}"' if span > 1 else ""
            node_attr = f' data-node-id="{html.escape(node_id)}"' if node_id else ""
            if c == 0:
                cls = "ingredient"
            elif content:
                cls = "op"
                ...
                cls = "empty"
            cells.append(f'<td{rowspan_attr}{node_attr} class="{cls}">{content}</td>')
...
def _banner_row(node: dict, n_cols: int) -> str:
    ...
    node_attr = f' data-node-id="{html.escape(node["id"])}"' if node.get("id") else ""
    return f'<tr><td colspan="{n_cols}"{node_attr} class="setup">{technique}{detail_html}</td></tr>'
